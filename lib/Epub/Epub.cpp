...
bool Epub::parseContentOpf(BookMetadataCache::BookMetadata& bookMetadata) {
  ...
  bookMetadata.thumbnail2bppHref = opfParser.thumbnail2bppPath;
...
std::string Epub::getFallbackChapterName(const int spineIndex) const {
  if (spineIndex < 0 || spineIndex >= static_cast<int>(getSpineItemsCount())) {
...
// Lightweight entry point used by the library browser: builds/loads the book
// cache (metadata incl. thumbnail href, spine, TOC) but skips CSS parsing.
bool Epub::loadMetadataOnly() { return load(true, /*skipLoadingCss=*/true); }

const std::string& Epub::getThumbnail2bppItem() const {
  static const std::string empty;
  if (!bookMetadataCache) return empty;
  return bookMetadataCache->coreMetadata.thumbnail2bppHref;
}
