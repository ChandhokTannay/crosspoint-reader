...
constexpr char MEDIA_TYPE_CROSSPOINT_THUMBNAIL[] = "application/x-crosspoint-thumbnail";
...
void XMLCALL ContentOpfParser::startElement(void* userData, const XML_Char* name, const XML_Char** atts) {
  ...
  if (self->state == IN_MANIFEST && (strcmp(name, "item") == 0 || strcmp(name, "opf:item") == 0)) {
    ...
    // Crosspoint's embedded 2bpp thumbnail; keep the first, ignore duplicates
    if (mediaType == MEDIA_TYPE_CROSSPOINT_THUMBNAIL) {
      if (self->thumbnail2bppPath.empty()) {
        self->thumbnail2bppPath = href;
      } else {
        LOG_DBG("COF", "Warning: Multiple Crosspoint thumbnails found. Ignoring duplicate: %s", href.c_str());
      }
    }
