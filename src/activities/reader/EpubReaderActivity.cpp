...
// Path on the SD card where completed book paths are recorded (one per line).
// The library browser reads this to show a "Read" indicator per book.
...
void markBookCompleted(const std::string& epubPath) {
  ...
  {
    HalFile in;
    if (Storage.openFileForRead("ERS", COMPLETED_BOOKS_FILE, in)) {
      std::string contents;
      char buf[129];
      int n;
      ...
      while (pos != std::string::npos) {
        ...
        const std::string line =
            (eol == std::string::npos) ? contents.substr(pos) : contents.substr(pos, eol - pos);
        if (line == epubPath) {
          return;  // Already marked as completed.
        }
        pos = (eol == std::string::npos) ? std::string::npos : eol + 1;
      }
      // Append by rewriting the file with the new line added.
      HalFile outFile;
      if (Storage.openFileForWrite("ERS", COMPLETED_BOOKS_FILE, outFile)) {
        contents += epubPath;
        contents += '\n';
  ...
  // File doesn't exist yet: create it with this book.
  HalFile outFile;
  if (Storage.openFileForWrite("ERS", COMPLETED_BOOKS_FILE, outFile)) {
    const std::string line = epubPath + "\n";
...
void EpubReaderActivity::render(RenderLock&& lock) {
  ...
  if (currentSpineIndex == epub->getSpineItemsCount()) {
    ...
    automaticPageTurnActive = false;
    // Mark this book as completed so the library can show it as "Read".
